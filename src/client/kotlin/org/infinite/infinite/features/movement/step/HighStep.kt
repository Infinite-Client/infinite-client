package org.infinite.infinite.features.movement.step
...
class HighStep : ConfigurableFeature() {
    ...
    override fun onTick() {
        ...
        if (!player.onGround() || player.onClimbable() || player.isInWater || player.isInLava) return
        // 水平方向の衝突のチェックは、ここではベロシティ予測に置き換えるため、一旦削除（または次のステップで再評価）
        // if (!player.horizontalCollision) return // これを削除

        // 2. 移動入力とジャンプチェック (既存と同様)
        if (player.input.moveVector.length() <= 1e-5f) return
        if (options.keyJump.isDown) return
        ...
        val currentBox = player.boundingBox.move(0.0, 0.05, 0.0).inflate(0.05) // わずかに調整
        val predictedCollisionBox = currentBox.move(velocity.x, 0.0, velocity.z)
        ...
        // 4. 段差の高さ検出と空間チェック
        // 衝突しているブロックの最大Y座標を検出
        val maxBlockY = predictedCollisions.maxOfOrNull { it.bounds().maxY } ?: Double.NEGATIVE_INFINITY
        ...
        if (!world.noCollision(player, currentBox.move(0.0, step, 0.0))) return
        ...
        player.setPosRaw(player.x + nextPredictedDiffX, player.y + step, player.z + nextPredictedDiffZ)
    }
}
