...
object WidgetRenderUtils {
    ...
    fun renderCustomBackground(widget: AbstractWidget, renderer: Graphics2DRenderer) {
        when (InfiniteClient.uiStyle) {
            UiStyle.Simple -> simpleStyle(widget, renderer)
            UiStyle.Carousel -> carouselStyle(widget, renderer)
        }
    }

    private fun carouselStyle(
        widget: AbstractWidget,
        renderer: Graphics2DRenderer,
    ) {
    ...
    private fun simpleStyle(widget: AbstractWidget, renderer: Graphics2DRenderer) {
        val theme = InfiniteClient.theme
        val colorScheme = theme.colorScheme
        ...
        val baseColor =
            colorScheme.surfaceColor.mix(colorScheme.backgroundColor, 0.5f)
