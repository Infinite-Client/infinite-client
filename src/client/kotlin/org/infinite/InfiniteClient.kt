...
object InfiniteClient : ClientModInitializer {
    ...
    fun reloadThemes() {
        val builtInThemes =
            listOf(
                org.infinite.gui.theme.official.InfiniteTheme(),
                org.infinite.gui.theme.official.SmeClanTheme(),
                org.infinite.gui.theme.official.HackerTheme(),
                org.infinite.gui.theme.official.PastelTheme(),
                org.infinite.gui.theme.official.MinecraftTheme(),
                org.infinite.gui.theme.official.CyberTheme(),
            )
        ...
        val parentDir =
            FabricLoader.getInstance().gameDir
                ...
                ?.resolve("themes")
                ?.toFile()
        val cwdThemes = CustomThemeLoader.load(java.io.File(".").absoluteFile.resolve("themes"))
        val customThemes = CustomThemeLoader.load(customDir)
        val rootThemes = CustomThemeLoader.load(rootDir)
        val parentThemes = parentDir?.let { CustomThemeLoader.load(it) } ?: emptyList()
        themes = builtInThemes + bundledThemes + customThemes + rootThemes + parentThemes + cwdThemes
